...
// captureWatcherV2 must feed a LIVE watcher edit into the v2 working log (so
// hook-less tools are v2-native), and SKIP a stale/replayed event (whose diff
// against the current file would mis-attribute).
func TestCaptureWatcherV2(t *testing.T) {
	...
	os.WriteFile(abs, []byte("h1\nh2\nai3\n"), 0o644)
	captureWatcherV2(Event{When: time.Now(), Tool: "copilot", GenType: "chat", RepoPath: repo, FilePath: "f.txt"})
	got := authorsByLine()
	...
	os.WriteFile(abs, []byte("h1\nh2\nai3\nx4\n"), 0o644)
	captureWatcherV2(Event{When: time.Now().Add(-time.Hour), Tool: "copilot", GenType: "chat", RepoPath: repo, FilePath: "f.txt"})
