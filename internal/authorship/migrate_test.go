...
func TestMigrateWorkingLogs(t *testing.T) {
	...
	// stays at baseA (amend/rebase re-flips it from there). dup.txt: a fresher log
	// already at baseB → the migrate must not overwrite it.
	...
	// committed.txt stays at baseA (kept for amend/rebase), not moved to baseB.
	if kept, _ := LoadWorkingLog(repo, "main", "baseA", "committed.txt"); kept == nil {
		t.Error("committed.txt should remain at baseA for amend/rebase re-flip")
	}
